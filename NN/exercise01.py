...
#    between standardization and normalization techniques, number of hidden neurons,
#    learning rate, momentum and regularization coefficients.
...
from .helpers import print_kfold_results
...
def main(file_path):
    ...
    print_kfold_results('logistic', fold_results_std, final_metrics_std)

    # Evaluate logistic regression with normalization
    X_train_norm, X_test_norm, fold_results_norm, final_metrics_norm = logistic_scaling(X_train, X_test, y_train, True)
    print_kfold_results('logistic', fold_results_norm, final_metrics_norm)

    # ------------------------- A2: Number of Hidden Neurons and Transformation Method Tuning ------------------------- #
    best_h1, best_result, _ = select_best_config_hidden(X_train_std, y_train)
    print_kfold_results('ann', best_result[0], best_result[1]) # from best config


    # ------------------------- A3: Hyperparameter Tuning (Learning Rate, Momentum) ------------------------- #
    best_lr, best_momentum, fold_results_hyper, final_metrics_hyper = select_best_config_hyper(X_train_std, y_train, best_h1)
    print_kfold_results('ann', fold_results_hyper, final_metrics_hyper)

    # ------------------------- A4: Regularization Coefficient Tuning ------------------------- #
    best_reg, fold_results_reg, final_metrics_reg = select_best_config_regularization(X_train_std, y_train, best_h1, best_lr, best_momentum)
    print_kfold_results('ann', fold_results_reg, final_metrics_reg)
    ...
    final_model = create_model_wrapper(
        'ann', X_train_std.shape[1], best_h1, best_lr, best_momentum, best_reg, simple_metrics=None
    )
    train_stats = train_model(
        model=final_model, X_train=X_train_sub, y_train=y_train_sub,
        X_val=X_val_sub, y_val=y_val_sub, model_type='ann'
    )
    ...
    # Return final performance summary
    best_summary = {
        'Selected Features': X_train_std.shape[1],
        'Validation Metrics': {
            'Loss': val_loss,
...
def main(file_path):
    ...
    train_stats = train_model(model=final_model, X_train=X_train_sub, y_train=y_train_sub, X_val=X_val_sub, y_val=y_val_sub, model_type='ann')
